#!/usr/bin/env python3
"""Generate ADADD V3 - Full-Stack Frontend (API-connected)"""

import json
import os

# Read current data to understand the API response structure
with open('/Users/mengjiachen/WorkBuddy/20260414101822/data/news_data.json', 'r', encoding='utf-8') as f:
    all_data = json.load(f)

print(f"Loaded {len(all_data)} news items for reference")
...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+SC:wght@300;400;500;700;900&family=Tencent+Sans:wght@400;500;700;900&display=swap" rel="stylesheet">
    <style>
...
.filter-row{display:flex;align-items:center;gap:24px;margin-bottom:14px;flex-wrap:wrap}
.filter-group-inline{display:flex;align-items:center;gap:8px}
.filter-group-inline.wide{flex:1;min-width:300px}
.filter-label-inline{font-size:13px;font-weight:500;color:var(--text-secondary);white-space:nowrap;flex-shrink:0}
.filter-btns,.cat-btns{display:flex;flex-wrap:wrap;gap:6px}
...
.impact-filters,.sort-options{display:flex;gap:4px}
...
@media(max-width:768px){
    ...
    .filter-row{gap:12px}
    .filter-group-inline.wide{min-width:auto}
...
<header class="navbar">
    <div class="navbar-inner">
        ...
        <div class="navbar-right">
            ...
            <div class="api-status connected" id="apiStatus"><span class="api-dot"></span>API已连接</div>
        </div>
    </div>
</header>
...
<section class="filter-bar">
    <div class="filter-row">
        <div class="filter-group-inline wide">
            <span class="filter-label-inline">来源：</span>
            <div class="filter-btns" id="sourceFilters"></div>
        </div>
        <div class="filter-group-inline">
            <div class="impact-filters" id="impactFilters"></div>
    ...
    <div class="filter-row">
        <div class="filter-group-inline wide">
            ...
            <div class="cat-btns" id="catFilters"></div>
        </div>
    </div>
</section>
...
<footer class="footer">
    <div class="footer-inner">
        ...
        <span>广告行业资讯聚合平台 V3</span><span class="divider">|</span>
        <span>Powered by Flask API + Tencent Blue Theme</span><span class="divider">|</span>
...
<script>
// ============================================================
// ADADD V3 - Full-Stack Edition (API-Connected)
// ============================================================
...
async function loadAllData() {
    ...
    const [newsData, stats] = await Promise.all([
        apiGet('/news?size=10000'),
        apiGet('/stats')
    ]);
    
    if (newsData && Array.isArray(newsData.items)) {
        state.allData = newsData.items;
        ...
        // Build local indexes for fast filtering
        buildIndexes();
...
function buildIndexes() {
    ...
    state.allData.forEach(item => {
        // Sources
        if (!state.sources[item.source]) state.sources[item.source] = 0;
        state.sources[item.source]++;
        
        // Categories
        if (!state.categories[item.category]) state.categories[item.category] = 0;
        state.categories[item.category]++;
        
        // Years
        const y = item.date ? item.date.substring(0, 4) : '';
...
function updateApiStatus(connected) {
    ...
    el.innerHTML = '<span class="api-dot"></span>' + (connected ? 'API已连接' : 'API离线');
}
...
const SOURCE_SEARCH_URLS = {
    ...
    'industry-media':'https://www.google.com/search?q={q}+site:socialbeta.com+OR+site:adquan.com',
...
function getNewsUrl(item) {
    if (PREMIUM_URLS[item.id]) return PREMIUM_URLS[item.id];
    const tpl = SOURCE_SEARCH_URLS[item.source];
    if (!tpl) return '#';
    const q = encodeURIComponent(item.title.replace(/\[.*?\]/g,'').trim());
    return tpl.replace('{q}', item.title.replace(/\[.*?\]/g,'').trim());
}
...
function applyFilters() {
    ...
    switch(state.sortBy) {
        ...
        case 'impact-desc': data.sort((a,b) => { const m={high:3,medium:2,low:1}; return (m[b.impact]||0)-(m[a.impact]||0); }); break;
    }
...
function renderSources() {
    ...
    let html = '<button class="filter-btn active" data-source="">全部<span class="count">'+state.allData.length+'</span></button>';
    for (const [key, count] of Object.entries(state.sources).sort((a,b)=>b[1]-a[1])) {
        const cfg = SOURCE_CONFIG[key] || {name:key, icon:'📌', color:'#999'};
        html += '<button class="filter-btn" data-source="'+key+'" style="'+(state.filters.source===key?'':'')+'">'+
            '<span>'+cfg.icon+' '+cfg.name+'</span><span class="count">'+count+'</span></button>';
    }
...
function renderCategories() {
    ...
    const cats = Object.entries(state.categories).sort((a,b)=>b[1]-a[1]);
    let html = '<button class="cat-btn active" data-cat="">全部</button>';
    cats.forEach(([cat,count]) => {
        html += '<button class="cat-btn" data-cat="'+cat+'">'+cat+
...
function renderStats() {
    ...
    el.innerHTML =
        ...
        '<div class="stat-item"><div class="stat-number highlight">'+Object.keys(s.years||{}).length+'</div><div class="stat-label">覆盖年数</div></div>'+
        '<div class="stat-divider"></div>'+
...
function renderNews() {
    ...
    grid.style.display='none'; timeline.style.display='none'; empty.style.display='none';
    ...
    if (state.viewMode === 'cards') {
        ...
        timeline.innerHTML = renderTimeline(items);
    }
}

function renderCard(item) {
    const srcCfg = SOURCE_CONFIG[item.source] || {name:item.source,icon:'📌',color:'#999'};
    const impCfg = IMPACT_CONFIG[item.impact] || IMPACT_CONFIG.low;
    const url = getNewsUrl(item);
    return '<article class="news-card" data-id="'+item.id+'">'+
        '<div class="card-header">'+
            '<span class="news-source" style="background:'+srcCfg.color+'18;color:'+srcCfg.color+'">'+srcCfg.icon+' '+srcCfg.name+'</span>'+
            '<span class="news-date">'+item.date+'</span>'+
            '<span class="impact-badge" style="background:'+impCfg.bgColor+';color:'+impCfg.color+'">'+impCfg.label+'</span>'+
        '</div>'+
        ...
        '<div class="card-footer">'+
            ...
            '<div><a href="'+url+'" target="_blank" rel="noopener noreferrer" class="visit-link" onclick="event.stopPropagation()">访问原文 ↗</a></div>'+
        '</div>'+
    '</article>';
}
...
function renderTimeline(items) {
    ...
    Object.keys(groups).sort().reverse().forEach(year => {
        html += '<div class="timeline-group"><h3 class="timeline-year">'+year+'年</h3><div class="timeline-items">';
...
function renderPagination() {
    ...
    html += '<button class="page-btn" '+(cp<=1?'disabled':'')+' data-p="'+(cp-1)+'">‹ 上一页</button>';
    
    const range = getPageRange(cp, tp);
    range.forEach(p => {
        if (p === '...') { html += '<span class="page-dots">...</span>'; }
        else { html += '<button class="page-btn '+(p===cp?'active':'')+'" data-p="'+p+'">'+p+'</button>'; }
    });
    
    html += '<button class="page-btn" '+(cp>=tp?'disabled':'')+' data-p="'+(cp+1)+'">下一页 ›</button>';
    html += '</div>';
    container.innerHTML = html;
    
    container.querySelectorAll('.page-btn:not([disabled])').forEach(btn => {
        btn.addEventListener('click', () => { state.currentPage=parseInt(btn.dataset.p); renderNews(); renderPagination(); });
    });
}
...
function openDetail(id) {
    const item = state.allData.find(d=>d.id==id); if(!item)return;
    const srcCfg = SOURCE_CONFIG[item.source] || {name:item.source,icon:'📌',color:'#999'};
    const impCfg = IMPACT_CONFIG[item.impact] || IMPACT_CONFIG.low;
    const url = getNewsUrl(item);
    ...
    document.getElementById('modalContent').innerHTML =
        '<div class="detail-header">'+
            '<div class="detail-meta-top">'+
                '<span class="news-source large" style="background:'+srcCfg.color+'18;color:'+srcCfg.color+'">'+srcCfg.icon+' '+srcCfg.name+'</span>'+
                '<span class="impact-badge large" style="background:'+impCfg.bgColor+';color:'+impCfg.color+'">'+impCfg.label+'</span>'+
                '<span class="category-tag large">'+escHtml(item.category)+'</span>'+
            '</div>'+
            '<span class="detail-date">'+item.date+'</span>'+
            '<h2 class="detail-title">'+escHtml(item.title)+'</h2>'+
            '<p class="detail-summary">'+escHtml(item.summary)+'</p>'+
        '</div>'+
        '<div class="detail-body"><p>'+(item.body?escHtml(item.body):escHtml(item.summary))+'</p></div>'+
        '<div class="detail-tags">'+(item.tags||[]).map(t=>'<span class="tag">'+escHtml(t)+'</span>').join('')+'</div>'+
        '<div class="detail-actions">'+
            '<button class="action-btn visit-original-btn" onclick="window.open(\''+url+'\',\'_blank\')">访问原文 ↗</button>'+
            '<button class="action-btn secondary" onclick="shareArticle(\''+escHtml(item.title)+'\')" title="分享">分享</button>'+
...
function shareArticle(title) {
    if(navigator.share){ navigator.share({title:title,url:location.href}); }
        ...
        const inp=document.createElement('textarea'); inp.value=title+' - ADADD 广告资讯聚合';
...
function bindCardEvents() {
    ...
    document.querySelectorAll('.timeline-item[data-id]').forEach(item=>{
...
document.addEventListener('DOMContentLoaded', async function(){
    // Render static UI first
    const impactContainer=document.getElementById('impactFilters');
    ['','high','medium','low'].forEach(k=>{
        const cfg=k?IMPACT_CONFIG[k]:{label:'全部影响力',color:'transparent'};
        const b=document.createElement('button');
        b.className='impact-filter-btn'+(k?'':' active');
        b.textContent=cfg.label;
        b.dataset.impact=k;
        b.onclick=function(){state.filters.impact=k; this.parentElement.querySelectorAll('.impact-filter-btn').forEach(x=>x.classList.remove('active')); this.classList.add('active'); renderAll();};
        impactContainer.appendChild(b);
    });
    
    const sortContainer=document.getElementById('sortOptions');
    [{k:'date-desc',l:'最新优先'},{k:'date-asc',l:'最早优先'},{k:'impact-desc',l:'高影响力'}].forEach(({k,l})=>{
        const b=document.createElement('button');
        b.className='sort-btn'+(k==='date-desc'?' active':'');
        b.textContent=l;
        b.onclick=function(){state.sortBy=k; this.parentElement.querySelectorAll('.sort-btn').forEach(x=>x.classList.remove('active')); this.classList.add('active'); renderAll();};
        sortContainer.appendChild(b);
    });
    
    const viewToggle=document.getElementById('viewToggle');
    [{k:'cards',l:'📋 卡片'},{k:'timeline',l:'📅 时间轴'}].forEach(({k,l})=>{
        const b=document.createElement('button');
        b.className='toggle-btn'+(k==='cards'?' active':'');
        b.textContent=l;
        b.onclick=function(){state.viewMode=k; this.parentElement.querySelectorAll('.toggle-btn').forEach(x=>x.classList.remove('active')); this.classList.add('active'); renderAll();};
        viewToggle.appendChild(b);
    });
    
    // Year filter from API data
    const yearSelect=document.getElementById('yearFilter');
    yearSelect.addEventListener('change',()=>{state.filters.year=yearSelect.value;renderAll();});
    
    // Search
    const searchInput=document.getElementById('searchInput'), searchBar=document.getElementById('searchBar'),
          clearBtn=document.getElementById('clearSearch');
    let searchTimer=null;
    searchInput.addEventListener('input',()=>{
        clearBtn.style.display=searchInput.value.length?'flex':'none';
        clearTimeout(searchTimer); searchTimer=setTimeout(()=>{
            state.searchQuery=searchInput.value; renderAll();
        },300);
    });
    searchInput.addEventListener('keydown',e=>{
        if(e.key==='Enter'){
            ...
            if(q){
                // In full-stack mode, search is always local-first
                state.searchQuery=q; renderAll();
            }
        }
        if(e.key==='Escape'){searchInput.value='';state.searchQuery='';clearBtn.style.display='none';renderAll();}
    });
    clearBtn.addEventListener('click',()=>{searchInput.value='';state.searchQuery='';clearBtn.style.display='none';searchInput.focus();renderAll();});
    
    // Refresh
    document.getElementById('refreshBtn').addEventListener('click',function(){this.classList.add('spinning');loadAllData().then(()=>{this.classList.remove('spinning');showToast('✅ 数据已更新');});});
    ...
    // Populate year filter after data loads
    [...state.years].sort().reverse().forEach(y=>{
...
output_path = '/Users/mengjiachen/WorkBuddy/20260414101822/index.html'
with open(output_path, 'w', encoding='utf-8') as f:
    f.write(html)
print(f"✅ Generated full-stack index.html ({len(html)} chars) -> {output_path}")
