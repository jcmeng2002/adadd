...
function renderNews(){
  ...
  if(state.view==="timeline"){
    ...
    if(total>1){nh+=`<div class="pagination"><div class="pgn-inn">`;nh+=`<button class="pg-btn" data-p="${state.page-1}"${state.page<=1?" disabled":""}>‹</button>`;
      ...
      nh+=`<div class="pg-jump-wrap"><span class="pg-jump-label">跳至</span><input type="text" class="pg-jump-input" id="pageJumpInput" placeholder="${state.page}" title="输入页码后按 Enter 跳转"><span class="pg-jump-label">/ ${total}</span></div>`;
      nh+="</div></div>";}
...
newsSection.addEventListener("click",function(e){
  ...
  if(cld&&!e.target.closest(".visit-link]")&&!e.target.closest("[data-url]")){
    ...
    'newsSection.addEventListener("keydown",function(e){\n'
    '  if(e.target.id==="pageJumpInput"&&e.key==="Enter"){var val=parseInt(e.target.value);if(val&&val>0){var tP=Math.ceil(ALL_DATA.length/12);state.page=Math.min(Math.max(1,val),tP);renderNews();}e.target.value="";}\n'
    '});\n\n'
