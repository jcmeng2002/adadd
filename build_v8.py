...
css = r"""@font-face{font-family:"Tencent Sans";src:url("fonts/TencentSans-W7.woff2") format("woff2"),url("fonts/TencentSans-W7.otf") format("opentype");font-weight:100 900;font-style:normal;font-display:swap}
:root{--p:#0052D9;--pl:#E8F0FE;--pd:#1a3c6e;--ph:#2670F0;--a:#00A870;--w:#ED7B2D;--d:#E02020;--t1:#1a1a1a;--t2:#666666;--t3:#999999;--bc:#e5e5e5;--bg:#f5f7fa;--wh:#ffffff;--ss:0 1px 3px rgba(0,0,0,.06);--sm:0 4px 12px rgba(0,0,0,.08);--sl:0 8px 30px rgba(0,0,0,.12);--r1:6px;--r2:10px;--r3:16px;--ff:"Tencent Sans",system-ui,-apple-system,BlinkMacSystemFont,"Microsoft YaHei",sans-serif}
...
var SOURCES={weibo:{name:"微博",icon:"📱",color:"#E6162D"},zhihu:{name:"知乎",icon:"💡",color:"#0084FF"},wechat:{name:"微信公众号",icon:"📨",color:"#07C160"},github:{name:"GitHub",icon:"🐙",color:"#24292E"},"tech-media":{name:"技术社区",icon:"⚙️",color:"#ED7B2D"},"industry-media":{name:"行业媒体",icon:"📰",color:"#0052D9"},"36kr":{name:"36氪",icon:"🚀",color:"#0066CC"},"tencent-ima":{name:"腾讯IMA",icon:"🎯",color:"#0052D9"},"research":{name:"研究报告",icon:"📊",color:"#0052D9"},"gov":{name:"政府政策",icon:"🏛️",color:"#C41E3A"}};
...
function renderFilters(){
  ...
  var srcExpanded=(typeof state.srcExpanded!=="undefined"&&state.srcExpanded);
  var srcToggle=srcExpanded?'<span class="cat-toggle" data-a="srctoggle">▲ 收起</span>':'<span class="cat-toggle" data-a="srctoggle">▼ 展开 ('+srcKeys.length+'个)</span>';
  ...
  var catExpanded=(typeof state.catExpanded!=="undefined"&&state.catExpanded);
  var catToggle=catExpanded?'<span class="cat-toggle" data-a="cattoggle">▲ 收起</span>':'<span class="cat-toggle" data-a="cattoggle">▼ 展开 ('+cats.length+'个)</span>';
  ...
  filterBar.innerHTML=`<div class="frow"><div class="flab">📂 来源：</div><div class="btns${srcExpanded?'':' src-collapsed'}">${sb}${srcToggle}</div></div>`+
    `<div class="frow"><div class="flab">🏷 分类：</div><div class="btns cat-btns${catExpanded?'':' cat-collapsed'}">${cb}${catToggle}</div></div>`+
    `<div class="frow">
...
function renderStats(){
  ...
  var todayAll=0,todayFiltered=0;
  ALL_DATA.forEach(function(x){
    yrs[x.date.substring(0,4)]=1;srcs[x.source]=1;
    if(x.date===todayStr||x.date.indexOf(todayStr)===0){todayAll++}
  });
  d.forEach(function(x){
    if(x.date===todayStr||x.date.indexOf(todayStr)===0){todayFiltered++}
  });
  var todayBadge=todayAll>0?'<span class="st-new">+<span class="st-new-num">'+todayAll+'</span> 今日新增</span>':'';
  statsBar.innerHTML=`<div class="st-item"><div class="st-num">${ALL_DATA.length}</div><div class="st-lbl">总资讯数${todayBadge}</div></div><div class="st-div"></div><div class="st-item"><div class="st-num">${d.length}${todayFiltered>0?'<span class="st-new" style="margin-left:4px;padding:1px 6px;font-size:10px">+'+todayFiltered+'</span>':''}</div><div class="st-lbl">当前筛选</div></div><div class="st-div"></div><div class="st-item"><div class="st-num">${Object.keys(yrs).length}</div><div class="st-lbl">覆盖年份</div></div><div class="st-div"></div><div class="st-item"><div class="st-num">${Object.keys(srcs).length}</div><div class="st-lbl">数据来源</div></div>`;
}
...
function renderNews(){
  ...
  if(state.view==="timeline"){
    ...
    var ch="";var todayStr=new Date().toISOString().slice(0,10);
    pg.forEach(function(it){
      var sr=SOURCES[it.source]||{name:it.source,icon:"📌",color:"#999"},ip=IMPACTS[it.impact]||{label:it.impact,color:"#999"};
      var isNew=(it.date===todayStr||it.date.indexOf(todayStr)===0)?'<span class="new-badge">NEW</span>':'';
      ch+=`<div class="card" data-id="${it.id}" data-url="${it.url}">${isNew}<div class="chdr"><span class="src-tag" style="background:${sr.color}20;color:${sr.color}">${sr.icon}${sr.name}</span><span class="date-text">${it.date}${isNew?'':' '}</span><span class="imp-badge" style="background:${ip.color}20;color:${ip.color}">${ip.label}</span></div><h3 class="ctitle">${esc(it.title)}</h3><p class="csum">${esc(it.summary)}</p><div class="ctags">${it.tags.map(function(t){return`<span class="tag">${esc(t)}</span>`}).join('')}</div><div class="cftr"><span class="cat-tag">${esc(it.category)}</span><a class="visit-link" href="${it.url}" target="_blank">访问原文 ↗</a></div></div>`;
    });
...
filterBar.addEventListener("click",function(e){
  ...
  else if(a==="cattoggle"){state.catExpanded=!state.catExpanded;renderFilters();return;}
  else if(a==="srctoggle"){state.srcExpanded=!state.srcExpanded;renderFilters();return;}
...
html_parts = [
    ...
    '<link rel="preload" href="fonts/TencentSans-W7.woff2" as="font" type="font/woff2" crossorigin>',
